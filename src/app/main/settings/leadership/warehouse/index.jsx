...
function Index() {
  ...
  const [lengthPage, setLengthPage] = useState(warehouses.length)
  ...
  useEffect(() => {
    if (searchText.length !== 0) {
      setData(
        _.filter(warehouses, (item) => item.name.toLowerCase().includes(searchText.toLowerCase()))
      )
    } else {
      setData(warehouses)
    }
  ...
  const fetchData = (page, rowsPerPage, search) => {
    setLoading(true)
    setTimeout(() => {
      dispatch(getWarehouses({ page: page + 1, rowsPerPage, search })).then((response) => {
        setPage(page)
