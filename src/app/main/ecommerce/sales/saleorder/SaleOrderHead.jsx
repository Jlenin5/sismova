...
import { deleteSaleOrder, putSaleOrder, getMaxId, postSaleOrder } from '../store/saleorderSlice'
...
const SaleOrderHead = () => {
  ...
  const handleSaveProduct = async () => {
    if(getValues().id === null) {
      ...
      dispatch(postSaleOrder(quoteData))
    } else {
      const quoteData = getValues()
      dispatch(putSaleOrder(getValues()))
    }
    returnProducts()
  }

  function handleRemoveProduct() {
    dispatch(deleteSaleOrder(getValues().id)).then(() => {
      returnProducts()
    })
  }
      ...
      <div className="flex flex-col items-center sm:items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
        ...
        >
          ...
          >
            ...
            <span className="flex mx-4 font-medium">{t('purchase_orders')}</span>
          </Typography>
        </motion.div>
