...
import { getCurrencies, selectCurrency, selectCurrencySearchText } from '../store/currenciesSlice'
import { useState, useEffect } from 'react'
import { useDispatch, useSelector } from 'react-redux'

function Index() {
  const dispatch = useDispatch()
  const categories = useSelector(selectCurrency)
  ...
  const fetchData = (page, rowsPerPage, search) => {
    setLoading(true)
    setTimeout(() => {
      dispatch(getCurrencies({ page: page + 1, rowsPerPage, search })).then((response) => {
        setPage(page)
