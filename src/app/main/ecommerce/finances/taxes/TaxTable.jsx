...
import { getTaxes, selectTaxes, selectTaxesSearchText } from '../store/taxSlice'

function TaxTable(props) {
  const dispatch = useDispatch()
  const taxes = useSelector(selectTaxes)
  const searchText = useSelector(selectTaxesSearchText)
  ...
  useEffect(() => {
    if (searchText.length !== 0) {
      setData(
        _.filter(taxes, (item) => item.taxName.toLowerCase().includes(searchText.toLowerCase()))
      )
      setPage(0)
    } else {
      setData(taxes)
