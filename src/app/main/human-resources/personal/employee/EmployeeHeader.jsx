...
import { deleteEmployee, putEmployee, getMaxId, postEmployee } from '../store/employeeSlice'
...
const EmployeeHeader = (props) => {
  ...
  const empFirstName = watch('empFirstName')
  ...
  const handleSaveProduct = async () => {
    if(getValues().id === null) {
      ...
      dispatch(postEmployee(productData))
    } else {
      // const productData = getValues()
      // const formData = new FormData()
      // await Promise.all(productData.product_images.map(async (image, index) => {
      ...
      // }))
      // dispatch(postImage(formData))
      dispatch(putEmployee(getValues()))
    }
    returnEmployees()
  }

  function handleRemoveProduct() {
    dispatch(deleteEmployee(getValues().id)).then(() => {
      returnEmployees()
    })
  }
