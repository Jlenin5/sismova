...
import { deleteJobPosition, postJobPosition, putJobPosition } from '../store/jpSlice'
import CloseIcon from '@mui/icons-material/Close'
import IconButton from '@mui/material/IconButton'

function JPForm(props) {
  ...
  const handleSubmit = (e) => {
    ...
    form.id ? dispatch(putJobPosition(form)) : dispatch(postJobPosition(form))
    props.onClose()
    handleReset()
  }
  ...
  useEffect(() => {
    props.dataToEdit ? setForm(props.dataToEdit) : setForm(JPInterface)
  }, [dispatch, props.dataToEdit])

  return (
    <Dialog open={props.open}>
      <DialogTitle className="flex justify-between mt-10">
        <div>{!form.id ? t('register_job_position') : t('update_job_position')}</div>
        <IconButton
          aria-label="close"
          onClick={handleClose}
          sx={{
